...
#         self.root = root
#         self.name = StringVar()
#         self.lista = ["juan", "pedro", "maria", "tomas", "argemiro"]

#         self.entry = ttk.Combobox(self.root, values=self.lista, textvariable=self.name)

#         self.entry.pack()
#         self.entry.bind("<<ComboboxSelected>>", self.search)
    
#     def filt(self, *args):
#         a = len(self.name.get())
#         if  a > 0:
            
#             if (i[:a] == self.name.get() for i in self.lista):
#                 return True
#         return False

#     def search(self, *args):
#         print(list(filter(self.filt, self.lista)))
